# unison-deb: an apt repository for current Unison, with fsmonitor
...
| `unison-deb-keyring` | all | `/usr/share/keyrings/unison-deb.gpg`, `/etc/apt/sources.list.d/unison-deb.sources` |
...
`unison-deb-keyring` is `Architecture: all` but built once per suite, because
...
- **`--arch <arch>`**, which matches the pattern `*_all.deb` and `*_<arch>.deb`.
  ...
  with no extra work, which is what makes `unison-deb-keyring` installable
...
`/unison-deb.asc` and shipped inside `unison-deb-keyring`.
...
`unison-deb-keyring` shipping both old and new public keys, wait for clients to
...
bootstrap/{trixie,resolute}/unison-deb-keyring.deb
unison-deb.asc
state.json
index.html
```
...
1. Create the public GitHub repo `porelli/unison-deb`.
...
2. **Index generation has two silent-failure modes**, both invisible without a
   ...
   `unison-deb-keyring` out of a per-architecture index, breaking the bootstrap
