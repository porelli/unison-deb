# unison-deb Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-09-16-unison-deb-design.md` — read it before starting. The plan argues from it and does not restate its reasoning.
...
- **Package names:** `unison`, `unison-gtk`, `unison-deb-keyring`. The first two deliberately shadow the distro's.
...
│   ├── make-keyring-deb.sh            build unison-deb-keyring for one suite
...
REPO_NAME="unison-deb"
REPO_ORIGIN="unison-deb"
REPO_LABEL="unison-deb"
REPO_URL="https://porelli.github.io/unison-deb"
...
KEYRING_PKG="unison-deb-keyring"
KEYRING_FILE="unison-deb.gpg"
SOURCES_FILE="unison-deb.sources"
```
...
```bash
tea repos create --login claude --name unison-deb --description "apt repository for current Unison with fsmonitor" --private=false
git remote add gitea https://gitea.porelli.eu/claude/unison-deb.git
...
git push gitea main
tea workflow run --login claude --repo claude/unison-deb probe
```
...
git push gitea main
tea workflow run --login claude --repo claude/unison-deb release -f version=2.54.0
```
...
- Produces: `scripts/make-keyring-deb.sh <codename> <pubkey-file> <outdir>` → `<outdir>/unison-deb-keyring_<krev>~<vtag>_all.deb`.
...
```bash
gpg --batch --quick-generate-key "unison-deb repository signing key <Linux571@gmail.com>" ed25519 sign never
...
gpg --armor --export "$KEYID" > /tmp/unison-deb.asc
gpg --armor --export-secret-keys "$KEYID" > /tmp/unison-deb-secret.asc
```
...
```bash
# Gitea
tea secret create --login claude --repo claude/unison-deb APT_SIGNING_KEY --value "$(cat /tmp/unison-deb-secret.asc)"
```
...
cp /tmp/unison-deb.asc packaging/keys/unison-deb.asc
rm -f /tmp/unison-deb-secret.asc /tmp/unison-deb.asc
```
...
```yaml
      - name: Build the keyring package
        run: |
          ...
          sh scripts/make-keyring-deb.sh "${{ matrix.suite }}" packaging/keys/unison-deb.asc "$PWD/out"
```

Run it: `tea workflow run --login claude --repo claude/unison-deb release -f version=2.54.0`
      ...
      - name: Build the keyring packages and the bootstrap tree
        run: |
          ...
          for suite in $(sh -c '. scripts/lib.sh; suite_list'); do
            ...
            sh scripts/make-keyring-deb.sh "$suite" packaging/keys/unison-deb.asc "repo/bootstrap/$suite"
            cp "repo/bootstrap/$suite"/*.deb "repo/pool/$suite/main/u/unison/"
          done
          cp packaging/keys/unison-deb.asc repo/unison-deb.asc
```
...
(`unison-deb-keyring_1~deb13_all.deb`), which no documented one-liner can predict.
...
```bash
tea repos edit --login claude --repo claude/unison-deb  # ensure BUILD_ARCHES=amd64 is set as a variable
```
...
git push gitea main
tea workflow run --login claude --repo claude/unison-deb release -f publish=false
```
...
```bash
tea workflow run --login claude --repo claude/unison-deb release
```
...
```bash
tea workflow run --login claude --repo claude/unison-deb release -f version=2.53.7
```
...
git push gitea main
tea workflow run --login claude --repo claude/unison-deb qemu-arm64 -f version=2.54.0
```
...
- Produces: a live repository at `https://porelli.github.io/unison-deb`.

- [ ] **Step 1: Create the public repository and push**

```bash
gh repo create porelli/unison-deb --public \
  --description "apt repository for the current Unison release, with unison-fsmonitor"
git remote add github https://github.com/porelli/unison-deb.git
...
```bash
gh secret set APT_SIGNING_KEY --repo porelli/unison-deb < /path/to/unison-deb-secret.asc
```
...
```bash
gpg --armor --export-secret-keys "$KEYID" | gh secret set APT_SIGNING_KEY --repo porelli/unison-deb
```
...
```bash
gh workflow run release --repo porelli/unison-deb
gh run watch --repo porelli/unison-deb
```
...
```bash
curl -fsS https://porelli.github.io/unison-deb/state.json
curl -fsS https://porelli.github.io/unison-deb/dists/trixie/InRelease | head -20
```
